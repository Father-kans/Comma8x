...
def check_car_battery_voltage(should_start, health, charging_disabled, msg):
  ...
  if charging_disabled and (health is None or health.health.voltage > (int(kegman_kans.conf['carVoltageMinEonShutdown'])+500)) and msg.thermal.batteryPercent < int(kegman_kans.conf['battChargeMin']):
    charging_disabled = False
    os.system('echo "1" > /sys/class/power_supply/battery/charging_enabled')
  elif not charging_disabled and (msg.thermal.batteryPercent > int(kegman_kans.conf['battChargeMax']) or (health is not None and health.health.voltage < int(kegman_kans.conf['carVoltageMinEonShutdown']) and not should_start)):
    charging_disabled = True
    os.system('echo "0" > /sys/class/power_supply/battery/charging_enabled')
  elif msg.thermal.batteryCurrent < 0 and msg.thermal.batteryPercent > int(kegman_kans.conf['battChargeMax']):
    charging_disabled = True
    os.system('echo "0" > /sys/class/power_supply/battery/charging_enabled')
