...
#    self.sR = [VM.sR, (VM.sR + float(kegman_kans.conf['sR_boost']))]
...
# add interpolated sR by GGamjang Niro
    ...
    if self.sR_delay_counter % 100 == 0:
      if self.v_ego_ed < v_ego:
        VM.sR = interp(v_ego, [27.5, 7], [VM.sR, VM.sR + 4.0])
      self.v_ego_ed = v_ego
    ...
    below_lane_change_speed = v_ego < self.alc_min_speed #LANE_CHANGE_SPEED_MIN
    ...
    if desire == log.PathPlan.Desire.laneChangeRight or desire == log.PathPlan.Desire.laneChangeLeft:
      ...
      self.libmpc.init_weights(MPC_COST_LAT.PATH / 3.0, MPC_COST_LAT.LANE, MPC_COST_LAT.HEADING, self.steer_rate_cost_prev)
    else:
      self.libmpc.init_weights(MPC_COST_LAT.PATH, MPC_COST_LAT.LANE, MPC_COST_LAT.HEADING, self.steer_rate_cost_prev)
